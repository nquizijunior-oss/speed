...
export function Candidats() {
  const { candidats, updateCandidat } = useAppContext();
  ...
  const [tableHeaders, setTableHeaders] = useState({
    ...
    age: getStoredString('candidatsTableHeaderAge', 'Âge'),
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem('candidatsTableHeaderAge', tableHeaders.age);
  ...
  const updateCandidate = (id: string, field: string, value: string) => {
    const numericFields = new Set(['age', 'heuresEffectuees', 'heuresPrevues', 'soldeDu']);
    const updates: Record<string, unknown> = {
      [field]: numericFields.has(field) ? Number(value.replace(/[^0-9.-]/g, '')) || 0 : value,
    };

    if (field === 'statut' && !defaultFilters.includes(value as StatutCandidat)) return;
    if (field === 'categorie' && !['A', 'A2', 'B', 'BE', 'C'].includes(value)) return;

    updateCandidat(id, updates as Partial<Candidat>);
  };

  const visibleCandidates = useMemo(() => candidats, [candidats]);
            ...
            <table className="w-full min-w-[900px] text-left text-sm">
              <thead>
                <tr className="border-b border-line text-xs uppercase tracking-wide text-ink-500">
                  ...
                  <th scope="col" className="px-6 py-3 font-semibold"><InlineEditableField value={tableHeaders.age} onSave={(next) => setTableHeaders((current) => ({ ...current, age: next }))} className="inline-block" /></th>
                ...
                {liste.map((candidate) => (
                  <tr key={candidate.id} className="transition-colors duration-150 hover:bg-canvas">
                    ...
                    <td className="px-6 py-4 tabular-nums text-ink-700">
                      <InlineEditableField value={`${candidate.age}`} onSave={(next) => updateCandidate(candidate.id, 'age', next)} className="inline-block" /> ans
                    </td>
                    <td className="px-6 py-4 tabular-nums text-ink-700">
